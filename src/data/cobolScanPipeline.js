/** Logical horizon-re scan pipeline steps for legacy COBOL claims workspace */
...
export function buildPipelineLog(message, kind = 'info') {
  return { ts: new Date().toISOString(), message, kind }
}
...
export function isCobolScanResult(result) {
  const langs = result?.stats?.languages ?? []
  return langs.some((l) => /cobol|jcl|copybook/i.test(l))
}
